...
Adapted from upstream (03705ac) for our _init_k8s_clients() which returns
a (CoreV1Api, NetworkingV1Api) tuple instead of a single CoreV1Api.
"""
...
def _load_provisioner_module():
    ...
    spec = importlib.util.spec_from_file_location("provisioner_app_kc", module_path)
    assert spec is not None
    assert spec.loader is not None
    module = importlib.util.module_from_spec(spec)
    sys.modules["provisioner_app_kc"] = module
...
def test_init_k8s_clients_rejects_directory_path(tmp_path):
    ...
    try:
        provisioner_module._init_k8s_clients()
        raise AssertionError("Expected RuntimeError for directory kubeconfig path")
    except RuntimeError as exc:
        assert "expected a file" in str(exc)


def test_init_k8s_clients_uses_file_kubeconfig(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        provisioner_module.k8s_client,
        "NetworkingV1Api",
        lambda *args, **kwargs: "networking-v1",
    )

    provisioner_module.KUBECONFIG_PATH = str(kubeconfig_file)

    result = provisioner_module._init_k8s_clients()

    assert called["config_file"] == str(kubeconfig_file)
    # Our version returns a tuple (CoreV1Api, NetworkingV1Api)
    assert result == ("core-v1", "networking-v1")


def test_init_k8s_clients_falls_back_to_incluster_when_missing(tmp_path, monkeypatch):
