...
**Sources**: open-ptc-agent, OpenClaw, Claude Code, Anthropic Engineering Blog (Context Engineering, Advanced Tool Use, Code Execution with MCP), OpenAI Deep Research, Perplexity, LangGraph, CrewAI, AutoGen, Gemini Deep Research, Grok/xAI, Manus AI, DSPy, OpenAI Agents SDK
...
| No skill-adaptive loading              | Skills listed statically regardless of task type    | Claude Code, open-ptc-agent        |
...
The conceptual foundation comes from Anthropic's "Advanced Tool Use" and "Code Execution with MCP" engineering blog posts, which describe how loading all tool definitions upfront creates severe context bloat (e.g., 35 GitHub tools consume ~26K tokens, 11 Slack tools ~21K tokens -- at scale this reaches 134K+ tokens of tool definitions alone). open-ptc-agent provides a provider-agnostic implementation of these concepts.

**Three-level progressive disclosure**:

1. **Level 1 -- Name only**: At startup, inject only tool names grouped by server (minimal tokens)
2. **Level 2 -- Name + description**: On first reference to a server, load one-line descriptions via a `search_tools(query, detail="summary")` function
3. **Level 3 -- Full schema**: On actual use, load complete signatures on-demand via documentation files

**open-ptc-agent implementation**: Tools are organized in a filesystem hierarchy. The agent discovers exact signatures via `glob("tools/docs/{server}/*.md")` -> `read_file("{tool}.md")`. MCP server tools are auto-converted to Python module documentation that the agent navigates naturally.

**Quantitative impact**: Anthropic reports that deferred tool loading reduced tool definition tokens from ~77K to ~8.7K (89% reduction) and improved task accuracy from 49% to 74% (Opus 4) and 79.5% to 88.1% (Opus 4.5) by reducing context noise. The open-ptc-agent implementation achieves comparable savings in a provider-agnostic manner.

**Recommendation**: For MCP servers with 5+ tools, implement three-level progressive disclosure:

1. Inject only server-level summaries (server name, tool count, import path) in the system prompt
2. Implement a `search_tools(query, detail_level)` utility that returns tool information at configurable granularity
3. Keep 3-5 most frequently used tools always-loaded; defer the rest
4. Use filesystem-based discovery (the open-ptc-agent pattern) so this works with any LLM provider
...
#### F. Persistent Skill Development (Anthropic Code Execution with MCP / open-ptc-agent Pattern)

Anthropic's "Code Execution with MCP" blog describes a pattern where agents persist reusable code as "Skills" -- functions and workflows that accumulate domain expertise over time. Rather than solving the same data processing challenge from scratch each session, agents save proven solutions and reload them in future executions.

**open-ptc-agent implementation**: The agent maintains a `skills/` directory containing domain-specific expertise folders (e.g., `skills/pdf_processing/`, `skills/financial_modeling/`, `skills/spreadsheet_analysis/`). Each skill folder contains reusable Python functions with an accompanying `SKILL.md` documentation file. The skills loader dynamically scans and registers available skills based on the current task context.

**Skill lifecycle**:

1. **Creation**: Agent writes a reusable function during task execution and saves it to the skills directory
2. **Documentation**: Agent generates a `SKILL.md` describing inputs, outputs, and usage patterns
3. **Discovery**: At task start, skills loader scans for relevant skills based on task type and injects their documentation
4. **Invocation**: Agent imports and calls skill functions from within sandbox code execution
5. **Evolution**: Skills can be updated when improved approaches are discovered

**Recommendation**:

1. Create a `skills/` directory structure for domain-specific reusable code
2. Implement a skills loader that scans for relevant skills based on the current task context (not loading all skills statically)
3. Allow the agent to save new skills during execution with accompanying documentation
4. Integrate skill discovery with progressive tool discovery -- skills are another class of on-demand capability
...
The conceptual foundation comes from Anthropic's "Advanced Tool Use" blog post, which describes Programmatic Tool Calling -- having the LLM write code that orchestrates tools rather than making sequential JSON tool calls. open-ptc-agent provides the provider-agnostic open-source implementation: the LLM writes Python code that orchestrates entire workflows in a sandbox, with MCP tools auto-converted to importable Python modules (e.g., `from tools.yfinance import get_stock_history`).

**Three key advantages over sequential tool calling**:

1. **Token efficiency**: Raw data stays in the sandbox; only summaries return to context. open-ptc-agent achieves 85-98% token reduction for data-heavy tasks. Anthropic reports 37% savings even on general complex tasks (43,588 -> 27,297 tokens).
2. **Control flow efficiency**: Loops, conditionals, and polling logic execute natively in the code environment rather than requiring model roundtrips between each tool call. This eliminates unnecessary inference passes.
3. **Data isolation**: Sensitive data (PII, financial records) can be processed entirely within the sandbox without flowing through the model's context -- a critical property for enterprise and regulated deployments.

**Recommendation**: For data-intensive workflows (ERP analysis, financial research), implement a `code_execution` tool that:

1. Runs Python in a secure sandbox (open-ptc-agent uses Daytona)
2. MCP tools are available as importable functions (auto-generated from MCP server schemas)
3. Only summaries/results return to context
4. Full data persists in sandbox files for further processing
5. Prioritize parallel-safe, idempotent operations for code execution
6. Document return formats explicitly so the LLM writes correct parsing logic
...
#### F. Tool Use Examples in Definitions (Anthropic Advanced Tool Use / open-ptc-agent Pattern)

Anthropic's "Advanced Tool Use" blog describes a technique of embedding concrete usage examples directly in tool definitions rather than relying solely on JSON schemas. Testing showed this improved accuracy from 72% to 90% on complex parameter handling -- particularly for tools with optional fields, polymorphic parameters, or API-specific conventions.

**Why schemas alone are insufficient**: JSON schemas describe *what* parameters exist but not *how* they should be used together. Complex tools often have interdependent parameters, conditional fields, or idiomatic usage patterns that schemas cannot express.

**open-ptc-agent implementation**: Tool documentation files (the `.md` files discovered via progressive tool discovery) contain usage examples showing minimal, partial, and full specification patterns. Since these are prompt-injected rather than API-level features, they work with any LLM provider.

**Recommendation**: For each tool with 3+ parameters or complex usage patterns:

1. Create 1-5 realistic examples showing common invocation patterns
2. Include a minimal example (required params only), a typical example, and a full example (all params)
3. Embed examples in the tool documentation files used by progressive discovery
4. Focus examples on ambiguous usage patterns where the schema alone is insufficient

```markdown
<!-- Example: tools/docs/tavily/search.md -->
## tavily_search
...
**Minimal**: `tavily_search(query="Python MCP server tutorial")`

**With depth**: `tavily_search(query="EU AI Act compliance requirements", search_depth="advanced", max_results=10)`
```
...
#### E. Privacy-Preserving Data Processing in Sandbox (Anthropic Code Execution with MCP / open-ptc-agent Pattern)

Anthropic's "Code Execution with MCP" blog highlights a critical benefit of sandbox-based data processing: sensitive data never needs to reach the model's context. When the agent processes data via code execution in the sandbox, intermediate results containing PII (emails, phone numbers, financial records) stay local. Only aggregated summaries or anonymized outputs flow back to the model.

**open-ptc-agent implementation**: The sandbox processes raw data from MCP tools (e.g., ERP records, financial data) entirely locally. The agent writes Python code that filters, aggregates, and summarizes -- with only the processed output returning to context. For example, processing 10,000 customer records to produce a 5-row summary means 9,995 records with PII never enter the model's context window.

**Additional techniques**:

- **PII tokenization**: Replace identifiers (emails, phone numbers, account numbers) with tokens before any data crosses the sandbox boundary. The tokens map back to real values only within the sandbox environment, so downstream tool calls (e.g., writing to a CRM) can resolve them without the model ever seeing raw PII.
- **Result filtering**: Apply allowlist/denylist patterns to tool outputs before they enter context, stripping sensitive fields at the sandbox boundary.

**Recommendation**: For enterprise and regulated deployments:

1. Route all data-heavy tool calls through sandbox code execution rather than direct tool invocation
2. Implement PII tokenization at the sandbox boundary for any data flowing back to context
3. Define per-tool sensitivity classifications that determine whether results can enter context directly or must be processed in the sandbox first
4. Log data flow patterns for audit compliance (what data entered the sandbox, what summaries were returned)
...
### Adoption Strategy: Diagnose First, Then Layer

Per Anthropic's "Advanced Tool Use" guidance, diagnose your primary bottleneck before selecting techniques. The three bottleneck categories map to different solutions:

- **Context bloat from tool definitions** -> Progressive tool discovery (Section 4.3E)
- **Large intermediate results consuming context** -> Code-as-Tool-Call / sandbox processing (Section 6.2B)
- **Parameter errors and tool misuse** -> Tool use examples in definitions (Section 6.2F)

Layer strategically: address the dominant bottleneck first, measure impact, then add the next technique. Implementing all three simultaneously increases integration complexity without proportional benefit.
...
| 9   | Progressive tool discovery       | 3-level disclosure + search_tools + on-demand | 3-4 days | 89% token savings on tool definitions |
...
| 21  | Privacy-preserving data processing | PII tokenization at sandbox boundary      | 3-4 days | Enterprise/regulated compliance |
| 22  | Persistent skill development       | Skills directory + loader + documentation | 3-4 days | Accumulated domain expertise    |
...
6. Add 1-5 usage examples to tool documentation files for complex tools (72% -> 90% accuracy improvement per Anthropic)
...
1. **open-ptc-agent** -- [https://github.com/Chen-zexi/open-ptc-agent](https://github.com/Chen-zexi/open-ptc-agent)
  Provider-agnostic open-source implementation of the concepts described in Anthropic's "Advanced Tool Use" and "Code Execution with MCP" blog posts (sources 5 & 6 below). Implements: Programmatic Tool Calling as sandbox code execution with auto-generated MCP-to-Python modules, three-level progressive tool discovery via filesystem navigation, tool use examples in documentation files, persistent skill development, think-after-search research loop, Jinja2 prompt composition, background subagent orchestration
2. **OpenClaw** -- [https://github.com/openclaw/openclaw](https://github.com/openclaw/openclaw)
...
3. **Anthropic Context Engineering Guide** -- [https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents)
...
4. **Claude Code System Prompts** -- [https://github.com/Piebald-AI/claude-code-system-prompts](https://github.com/Piebald-AI/claude-code-system-prompts)
...
5. **Anthropic Advanced Tool Use** -- [https://www.anthropic.com/engineering/advanced-tool-use](https://www.anthropic.com/engineering/advanced-tool-use)
  Conceptual foundation for three techniques: Tool Search (deferred tool loading for 89% token reduction, 49%->74% accuracy), Programmatic Tool Calling (code-based tool orchestration for 37% token savings), Tool Use Examples (embedded usage examples for 72%->90% accuracy). Note: the blog describes Anthropic-native API features; open-ptc-agent (source 1) provides the provider-agnostic implementation
6. **Anthropic Code Execution with MCP** -- [https://www.anthropic.com/engineering/code-execution-with-mcp](https://www.anthropic.com/engineering/code-execution-with-mcp)
  Filesystem-based tool discovery, data filtering in execution environments (98.7% token savings: 150K->2K), progressive disclosure via configurable search_tools detail levels, PII tokenization at sandbox boundary, persistent skill development. Note: open-ptc-agent (source 1) provides the provider-agnostic implementation

### Secondary Sources (Researched)

1. **Manus AI** -- [https://manus.im/blog/Context-Engineering-for-AI-Agents-Lessons-from-Building-Manus](https://manus.im/blog/Context-Engineering-for-AI-Agents-Lessons-from-Building-Manus)
  KV-cache optimization, three-file state system, attention anchoring, context-aware state machine, PDCA cycle
2. **OpenAI Deep Research** -- RL-trained ReAct loops, clarification pipeline, background execution
3. **Perplexity / DRACO Benchmark** -- Sequential plan execution, citation scoring, research quality evaluation
4. **Google Gemini Deep Research** -- Async task manager, multi-pass self-critique, 1M token context + RAG
5. **Grok/xAI** -- Multi-agent internal architecture (4-16 agents), 2M token context
6. **LangGraph** -- Reducer-driven state, checkpointing, scatter-gather multi-agent
7. **CrewAI** -- Role-based crews, event-driven Flows, hierarchical process
8. **AutoGen / Microsoft Agent Framework** -- Conversable agents, YAML declarative definitions
9. **Claude Agent SDK** -- Two-agent harness, progress file pattern, hooks system
